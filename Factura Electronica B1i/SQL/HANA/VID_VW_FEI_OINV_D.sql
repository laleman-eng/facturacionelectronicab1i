DROP VIEW VID_VW_FEI_OINV_D 
CREATE VIEW VID_VW_FEI_OINV_D 
AS 
SELECT
T0."FolioNum" "FolioNum" ,
CASE WHEN T0."DocSubType" IN ('IB', 'EB') 
				THEN CASE WHEN T1."VatSum" = 0.0 
				THEN 1 
				ELSE 0 
				END 
				ELSE CASE WHEN T1."VatSum" = 0 
				THEN 1 
				ELSE 2 
				END 
...
LEFT(IFNULL(T1."unitMsr", ''), 4) "DET_UNIDAD_MEDIDA" ,
	CASE WHEN T0."DocType" = 'S' 
				...
				ELSE T1."Quantity" 
				END "Quantity" ,
	CASE WHEN T0."DocSubType" = 'IX' OR RIGHT(N0."BeginStr", 3) = '110' OR 	RIGHT(N0."BeginStr", 3) = '111' 
				THEN ROUND(T1."PriceBefDi",	4) WHEN T0."DocSubType" IN ('IB', 'EB') 
				THEN CASE IFNULL(T1."Currency",	 'CLP') WHEN 'CLP' 
				THEN T1."PriceAfVAT" WHEN '$' 
				THEN T1."PriceAfVAT" 
				ELSE ROUND(T1."PriceAfVAT" * T1."Rate",	 4) 
				END 
				ELSE CASE IFNULL(T1."Currency", 'CLP') WHEN 'CLP' 
	...
	CASE WHEN IFNULL(T1."DiscPrcnt",0.0) <= 0.0 
				THEN 0.0 
				ELSE CASE WHEN T0."DocSubType" = 'IX' 	OR 	RIGHT(N0."BeginStr", 3) = '111' 
				THEN (ROUND(T1."PriceBefDi" * CASE WHEN T0."DocType" = 'S' 	THEN 1 
						ELSE T1."Quantity" 
						END - CASE T0."CurSource" WHEN 'L' 	THEN ROUND(T1."LineTotal",0) WHEN 'S' 
						THEN T1."TotalSumSy" WHEN 'C' 
						THEN T1."TotalFrgn" 
						END,4)) 
				ELSE ROUND((CASE WHEN IFNULL(T1."Currency", 'CLP') IN ('CLP', '$') 
				THEN T1."PriceBefDi" 
				ELSE ROUND(T1."PriceBefDi" * T1."Rate",	 2) END) * CASE WHEN T0."DocType" = 'S' THEN 1 	ELSE T1."Quantity" 	END,0) - T1."LineTotal" 
				END 
				END "DiscSum" ,	CASE WHEN IFNULL(T1."DiscPrcnt", 0.0) >= 0.0 THEN 0.0 	
				ELSE CASE WHEN T0."DocSubType" = 'IX' OR RIGHT(N0."BeginStr",	 3) = '111' 
				THEN (ROUND(T1."PriceBefDi" * CASE WHEN T0."DocType" = 'S' 	THEN 1 ELSE T1."Quantity"END - CASE T0."CurSource" WHEN 'L' 
						THEN ROUND(T1."LineTotal",	 0) WHEN 'S' 
						THEN T1."TotalSumSy" WHEN 'C' 
						THEN T1."TotalFrgn" 
						END,4)) 	
				...
				END 
				END "RecargoMonto" ,
	CASE WHEN T0."DocSubType" = 'IX' OR RIGHT(N0."BeginStr", 3) = '110' OR 	RIGHT(N0."BeginStr",3) = '111' 	THEN CASE T0."CurSource"
	WHEN 'L'  THEN ROUND(T1."LineTotal", 0) 
	WHEN 'S' THEN T1."TotalSumSy" 
	WHEN 'C' THEN T1."TotalFrgn" 
	END WHEN T0."DocSubType" IN ('IB', 'EB') THEN ROUND(T1."LineTotal" + T1."VatSum", 0) ELSE ROUND(T1."LineTotal", 0) 
	END "LineTotal" ,
