DROP PROCEDURE VID_SP_FEI_NOTACREDITO_DR;
CREATE PROCEDURE VID_SP_FEI_NOTACREDITO_DR
(
     IN DocEntry		Integer
...
BEGIN
	docs1 = SELECT T0."TpoMov"
				  ...
				  ,T0."IndExeDR"
				  ,T0."LineaOrden"
			  FROM VID_VW_FEI_NOTACREDITO_DR T0 
			 WHERE 1 = 1
	...
	docs2 = SELECT T0."TpoMov"
				  ...
				  ,T0."IndExeDR"
				  ,T0."LineaOrden"
			  FROM VID_VW_FEI_NOTACREDITO_DR_EXTRA T0 
			 WHERE 1 = 1
			   AND IFNULL(T0."ValorDR", 0) <> 0
			   AND T0."DocEntry" = :DocEntry
			   AND T0."ObjType"  = :ObjType;
