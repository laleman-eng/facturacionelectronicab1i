DROP PROCEDURE VID_SP_FEI_NOTACREDITO_IR;
CREATE PROCEDURE VID_SP_FEI_NOTACREDITO_IR
(
     IN DocEntry		Integer
...
BEGIN
	docs1 = SELECT T0."TipoImp"
				  ,T0."TasaImp"
				  ,T0."MontoImp"
			  FROM VID_VW_FEI_NOTACREDITO_IR T0 
			 WHERE 1 = 1
	...
	docs2 = SELECT T0."TipoImp"
				  ,T0."TasaImp"
				  ,T0."MontoImp"
			  FROM VID_VW_FEI_NOTACREDITO_IR_EXTRA T0 
			 WHERE 1 = 1
			   AND IFNULL(T0."MontoImp", 0) <> 0
			   AND T0."DocEntry" = :DocEntry
			   AND T0."ObjType"  = :ObjType;
