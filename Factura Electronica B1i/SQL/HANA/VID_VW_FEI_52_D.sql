DROP VIEW "VID_VW_FEI_52_D";  
CREATE VIEW "VID_VW_FEI_52_D"  
AS 
SELECT
	...
	ELSE T1."Quantity" 
	END "Quantity" ,
	CASE WHEN "U_Traslado" = 5 THEN 0 
	ELSE CASE IFNULL(T1."Currency",'CLP') WHEN 'CLP' THEN T1."PriceBefDi" 
	WHEN '$'THEN T1."PriceBefDi" ELSE ROUND(T1."PriceBefDi" * T1."Rate", 4) 
	END 
	END"Price" ,
	CASE WHEN T1."DiscPrcnt" >= 0 THEN T1."DiscPrcnt" 
	ELSE 0.0 
	END "DiscPrcnt" ,
...
SELECT T0."FolioNum" "FolioNum" ,
CASE WHEN T1."VatSum" = 0 
THEN 1 
ELSE 2 
END "Indicador_Exento" ,
...
ELSE T1."Quantity" 
END "Quantity" ,
CASE IFNULL(T1."Currency",'CLP') WHEN 'CLP' 
THEN T1."PriceBefDi" WHEN '$' 
THEN T1."PriceBefDi" 
...
THEN 0.0 
ELSE ROUND((CASE WHEN IFNULL(T1."Currency", 'CLP') IN ('CLP','$') 
		THEN T1."PriceBefDi" ELSE ROUND(T1."PriceBefDi" * T1."Rate", 2) 
		END) * CASE WHEN T0."DocType" = 'S' THEN 1 	ELSE T1."Quantity" 	END, 0) - T1."LineTotal" 
		END "DiscSum" ,
CASE WHEN IFNULL(T1."DiscPrcnt", 0.0) >= 0.0 
THEN 0.0 
ELSE T1."LineTotal" - ROUND((CASE WHEN IFNULL(T1."Currency",'CLP') IN ('CLP', '$') 
							THEN T1."PriceBefDi" ELSE ROUND(T1."PriceBefDi" * T1."Rate", 2)END) * 
							CASE WHEN T0."DocType" = 'S' THEN 1 ELSE T1."Quantity" END, 0) 
							END "RecargoMonto" ,
...
SELECT
	 ...
	 THEN T1."PriceBefDi" WHEN '$' 
THEN T1."PriceBefDi" 
ELSE ROUND(T1."PriceBefDi" * T1."Rate",	 4) END "Price" ,
	 CASE WHEN T1."DiscPrcnt" >= 0 THEN T1."DiscPrcnt" 
	 ELSE 0.0 
	 END "DiscPrcnt" ,
	 CASE WHEN IFNULL(T1."DiscPrcnt", 0.0) <= 0.0 THEN 0.0 ELSE ROUND((CASE WHEN IFNULL(T1."Currency",	 'CLP') IN ('CLP', '$') THEN T1."PriceBefDi" 
																		ELSE ROUND(T1."PriceBefDi" * T1."Rate", 2) END) * CASE WHEN T0."DocType" = 'S' 	THEN 1 	ELSE T1."Quantity" 	
																		END, 0) - T1."LineTotal" 
	END "DiscSum" ,
	 CASE WHEN IFNULL(T1."DiscPrcnt", 0.0) >= 0.0 THEN 0.0 ELSE T1."LineTotal" - ROUND((CASE WHEN IFNULL(T1."Currency",	 'CLP') IN ('CLP', '$') THEN T1."PriceBefDi" ELSE ROUND(T1."PriceBefDi" * T1."Rate", 2) 
	 END) * CASE WHEN T0."DocType" = 'S' 	THEN 1 	ELSE T1."Quantity" 	END, 0) 
	 END "RecargoMonto" ,
	 ROUND(T1."LineTotal", 0) "LineTotal" , IFNULL(F0."U_CodImpto", '') "CodImpAdic" ,	 
	LEFT(IFNULL(U0."U_NAME",''), 30) "Usuario" , 
	IFNULL(T4."TaxSum",	 0.0) "MontoImptoAdic" ,
	 IFNULL(F0."U_Porc", 0.0) "PorcImptoAdic" ,
	 T1."Rate" "Rate" ,
...
SELECT
	 ...
	 ELSE ROUND((CASE WHEN IFNULL(T1."Currency", 'CLP') IN ('CLP', '$') 
	 THEN T1."PriceBefDi" ELSE ROUND(T1."PriceBefDi" * T1."Rate", 2) 
	 END) * CASE WHEN T0."DocType" = 'S' THEN 1 ELSE T1."Quantity" 
	 END, 0) - T1."LineTotal" 
	 END "DiscSum" ,
	 CASE WHEN IFNULL(T1."DiscPrcnt", 0.0) >= 0.0 
	 THEN 0.0 ELSE T1."LineTotal" - ROUND((CASE WHEN IFNULL(T1."Currency", 'CLP') IN ('CLP', '$') 
	 THEN T1."PriceBefDi" 
	 ELSE ROUND(T1."PriceBefDi" * T1."Rate", 2) END) * CASE WHEN T0."DocType" = 'S' THEN 1 ELSE T1."Quantity" END, 0) END "RecargoMonto" ,
	 CASE WHEN "U_Traslado" = 5 THEN 0 ELSE ROUND(T1."LineTotal", 0) END "LineTotal",
	 IFNULL(F0."U_CodImpto", '') "CodImpAdic" ,LEFT(IFNULL(U0."U_NAME",	 ''),	 30) "Usuario" ,
	 IFNULL(T4."TaxSum", 0.0) "MontoImptoAdic" ,
	 IFNULL(F0."U_Porc", 0.0) "PorcImptoAdic" ,
	 T1."Rate" "Rate" ,
	 T1."Currency" "Currency" ,
...
JOIN "NNM1" N0 ON N0."Series" = T0."Series" 
AND N0."ObjectCode"= T0."ObjType" 
LEFT JOIN "WTQ4" T4 ON T4."DocEntry" = T0."DocEntry" 
AND T4."LineNum" = T1."LineNum" 
AND T4."StaCode" NOT IN ('IVA', 'IVA_EXE') 
LEFT JOIN "@VID_FEIMPADIC" F0 ON F0."Code" = T4."StaCode" 
WHERE 1 = 1 
...
LEFT(IFNULL(U0."U_NAME",''), 30) "Usuario" ,
	 ...
	 T0."DocEntry" "DocEntry" 
FROM "OWTQ" T0 
JOIN "WTQ10" T10 ON T10."DocEntry" = T0."DocEntry" 
JOIN "OUSR" U0 ON U0."INTERNAL_K" = T0."UserSign" 
JOIN "NNM1" N0 ON N0."Series" = T0."Series" 
AND N0."ObjectCode" = T0."ObjType" 
WHERE 1 = 1 
