DROP PROCEDURE VID_SP_FEI_Boleta_D;
CREATE PROCEDURE VID_SP_FEI_Boleta_D
(
     IN DocEntry		Integer
...
BEGIN
	SELECT ROW_NUMBER() OVER(ORDER BY T0."LineaOrden", T0."LineaOrden2")						"NroLinDet" 
		...
		,T0."Price"																				"PrcItem"
		--Comentado por no estar dentro del JSON
		--,0.0																					"PrcRef"
		,T0."Quantity"																			"QtyItem"
		--Comentado por no estar dentro del JSON
		--,0.0																					"QtyRef"
		...
		/*Valor del código tiene que ir dentro del nodo CdgItem junto con el nodo TpoCodigo
		"CdgItem": [
          {
            "TpoCodigo": "string",
            "VlrCodigo": "string"
		...
		*/
		...
		--Comentados por no existir dentro del JSON, boleta no lleva impuestos adicionales
